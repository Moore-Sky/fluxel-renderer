...
The page under test supplies a deliberately small, application-owned protocol::
    ...
    window.__fluxelEvidence = {
      ...
      loseContext: async () => void,
      restoreContext: async () => void,
...
also asks every WebGL canvas for ``getError()``.  A mock or a successful JS call
...
def renderer_observation(cdp: Cdp) -> dict[str, object]:
    """Collect browser-side GPU facts and generic WebGL errors without app internals."""
    expression = r"""(() => {
...
def reject_bad_snapshot(snapshot: object, label: str, *, visible: bool) -> None:
    ...
    for color in colors if isinstance(colors, list) else []:
        ...
        if visible and (expected is None or actual is None):
...
def reject_diagnostics(cdp: Cdp, observation: dict[str, object]) -> None:
    ...
    if bad:
        raise EvidenceError(f"browser/WebGL diagnostics are not clean: {bad!r}")
...
def capture_state(cdp: Cdp, output: Path, label: str, repeats: int, *, visible: bool) -> dict[str, object]:
    records = []
    previous_marker = 0
    ...
    def next_snapshot(sample_label: str) -> dict[str, object]:
        ...
        for _ in range(12):
            ...
            reject_bad_snapshot(value, sample_label, visible=visible)
            if not isinstance(value, dict):
    ...
    if visible:
        for index in range(15):
            time.sleep(0.02)
            dense.append(next_snapshot(f"{label}-dense-{index}"))
    return {
        "samples": records,
...
def scoped_measurements(states: dict[str, object], startup_ms: float) -> dict[str, object]:
    ...
    return {
        "scope": "retained Stage 1 scene in named Chrome WebGL2 target",
...
def main() -> int:
    ...
    output = Path(args.output).resolve() if args.output else repo / "target/evidence" / sha / "stage2"
    ...
    manifest: dict[str, object] = {"schema": 1, "started_at": utc_now(), "repo_sha": sha,
        ...
        "site_root": str(site_root), "entry": entry, "commands": command_log,
    ...
    try:
        with EvidenceHttpServer(site_root) as server, chrome_log.open("w", encoding="utf-8") as stderr:
            ...
            states["stable"] = capture_state(cdp, output, "stable", args.frames, visible=True)
            cdp.evaluate(protocol_call("resize", max(1, args.width // 2), max(1, args.height // 2)))
            states["resized"] = capture_state(cdp, output, "resized", args.frames, visible=True)
            cdp.evaluate(protocol_call("resize", 0, 0))
            states["zero_size"] = capture_state(cdp, output, "zero-size", args.frames, visible=False)
            cdp.evaluate(protocol_call("resize", args.width, args.height))
            states["restored"] = capture_state(cdp, output, "restored", args.frames, visible=True)
            cdp.evaluate(protocol_call("setVisibleForTest", False))
            states["hidden"] = capture_state(cdp, output, "hidden", args.frames, visible=False)
            cdp.evaluate(protocol_call("setVisibleForTest", True))
            states["visible"] = capture_state(cdp, output, "visible", args.frames, visible=True)
            cdp.evaluate(protocol_call("loseContext"))
            states["context_lost"] = capture_state(cdp, output, "context-lost", args.frames, visible=False)
            cdp.evaluate(protocol_call("restoreContext"))
            states["context_restored"] = capture_state(cdp, output, "context-restored", args.frames, visible=True)
            observation = renderer_observation(cdp)
        ...
        if process and process.poll() is None:
            ...
            try:
                ...
                process.kill()
    print(f"Stage 2 browser evidence passed: {output / 'manifest.json'}")
    return 0

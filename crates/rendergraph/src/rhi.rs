...
///
/// Ordinary texture and buffer import contracts require [`Self::Caller`].
/// [`Self::Surface`] is reserved for images declared through the separate
/// surface-import API, where the surface adapter controls acquisition and
/// release rather than a frame resource provider.
#[derive(Clone, Copy, Debug, Eq, Hash, PartialEq)]
#[non_exhaustive]
pub enum ExternalOwnership {
    /// The caller owns the resource and retains responsibility for its lifetime.
    Caller,
    /// The presentation surface owns an acquired image.
    Surface,
}

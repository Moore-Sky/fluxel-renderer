...
The `0.2.6` slice adds a separate encoded `Rgba8UnormSrgb` base-color upload and
fixed draw. Native sampling decodes each texel to linear before the existing
...
| Renderer `0.2` | In progress: GPU-ready mesh/texture/UV snapshots and fixed textured unlit draws → fixed private linear-clamp and sRGB decode → basic PBR → one real static scene |
